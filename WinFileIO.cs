...
namespace Win32FileIO
{
	public unsafe class WinFileIO : IDisposable
	{
		...
		public void OpenForReading(string FileName)
		{
			...
			this.Close();
			this.pHandle = CreateFile(FileName, GENERIC_READ, 0, 0, OPEN_EXISTING, 0, 0);
			if (this.pHandle == System.IntPtr.Zero)
			{
		...
		public void OpenForWriting(string FileName)
		{
			...
			this.Close();
			this.pHandle = CreateFile(FileName, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);
			if (this.pHandle == System.IntPtr.Zero)
			{
				Win32Exception WE = new Win32Exception();
		...
		public int Read(int BytesToRead)
		{
			...
			int BytesRead = 0;
			if (!ReadFile(this.pHandle, this.pBuffer, BytesToRead, &BytesRead, 0))
			{
				Win32Exception WE = new Win32Exception();
		...
		public int ReadUntilEOF()
		{
			...
			byte* pBuf = (byte*)this.pBuffer;
			// Do until there are no more bytes to read or the buffer is full.
			for (; ; )
			{
				if (!ReadFile(this.pHandle, pBuf, BlockSize, &BytesReadInBlock, 0))
				{
		...
		public int ReadBlocks(int BytesToRead)
		{
			...
			do
			{
				...
				if (!ReadFile(this.pHandle, pBuf, BlockByteSize, &BytesReadInBlock, 0))
				{
					Win32Exception WE = new Win32Exception();
		...
		public int Write(int BytesToWrite)
		{
			...
			if (!WriteFile(this.pHandle, this.pBuffer, BytesToWrite, &NumberOfBytesWritten, 0))
			{
				Win32Exception WE = new Win32Exception();
		...
		public int WriteBlocks(int NumBytesToWrite)
		{
			...
			do
			{
				...
				if (!WriteFile(this.pHandle, pBuf, BytesToWrite, &BytesWritten, 0))
				{
					// This is an error condition.  The error msg can be obtained by creating a Win32Exception and
					// using the Message property to obtain a description of the error that was encountered.
					Win32Exception WE = new Win32Exception();
		...
		public bool Close()
		{
			...
			{
				Success = CloseHandle(this.pHandle);
				this.pHandle = IntPtr.Zero;
			}
			return Success;
		}
