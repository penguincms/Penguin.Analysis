...
namespace Penguin.Analysis
{
    public unsafe class WinFileIO : IDisposable
    {
        ...
        private IntPtr pHandle;      // Handle to the file to be read from or written to.
        ...
        public void OpenForReading(string FileName)
        {
            ...
            {
                Win32Exception WE = new();
                ApplicationException AE = new("WinFileIO:OpenForReading - Could not open file " +
                  FileName + " - " + WE.Message);
                throw AE;
            }
        }
        ...
        public void OpenForWriting(string FileName)
        {
            ...
            {
                Win32Exception WE = new();
                ApplicationException AE = new("WinFileIO:OpenForWriting - Could not open file " +
                    FileName + " - " + WE.Message);
                throw AE;
            }
        }
        ...
        public int Read(int BytesToRead)
        {
            ...
            {
                Win32Exception WE = new();
                ApplicationException AE = new("WinFileIO:Read - Error occurred reading a file. - " +
                    WE.Message);
                throw AE;
            }
            return BytesRead;
        ...
        public int ReadUntilEOF()
        {
            ...
            for (; ; )
            {
                if (!ReadFile(pHandle, pBuf, BlockSize, &BytesReadInBlock, 0))
                {
                    ...
                    Win32Exception WE = new();
                    ApplicationException AE = new("WinFileIO:ReadUntilEOF - Error occurred reading a file. - "
                        + WE.Message);
                    throw AE;
                }
        ...
        public int ReadBlocks(int BytesToRead)
        {
            ...
            do
            {
                ...
                {
                    Win32Exception WE = new();
                    ApplicationException AE = new("WinFileIO:ReadBytes - Error occurred reading a file. - "
                        + WE.Message);
                    throw AE;
                }
                if (BytesReadInBlock == 0)
        ...
        public int Write(int BytesToWrite)
        {
            ...
            {
                Win32Exception WE = new();
                ApplicationException AE = new("WinFileIO:Write - Error occurred writing a file. - " +
                    WE.Message);
                throw AE;
            }
        ...
        public int WriteBlocks(int NumBytesToWrite)
        {
            ...
            do
            {
                ...
                if (!WriteFile(pHandle, pBuf, BytesToWrite, &BytesWritten, 0))
                {
                    ...
                    // using the Message property to obtain a description of the error that was encountered.
                    Win32Exception WE = new();
                    ApplicationException AE = new("WinFileIO:WriteBlocks - Error occurred writing a file. - "
                        + WE.Message);
                    throw AE;
                }
                pBuf += BytesToWrite;
